...
# Load the CSV file
file_path = r"C:\Users\admin\Downloads\New foldhh\pmos_ft.csv"  # Update with the actual file name
...
for length in lengths:
    ...
    if ft_col in pmos_data.columns and gmid_col in pmos_data.columns:
        # Convert fT from Hz to GHz by dividing by 1e9
        plt.plot(pmos_data[gmid_col], pmos_data[ft_col] / 1e9, label=f"L = {length}nm")

# Add labels and grid
plt.xlabel(r"$g_m / I_D$")
plt.ylabel(r"$f_T$ (GHz)")  # Y-axis label in GHz
plt.title("PMOS: $f_T$ vs $g_m / I_D$")
plt.grid(True)
