...
for length in lengths:
    ...
    if idw_col in pmos_data.columns and gmid_col in pmos_data.columns:
        plt.plot(pmos_data[gmid_col], pmos_data[idw_col], label=f"L = {length}nm")

# Add labels and grid
plt.xlabel(r"$g_m / I_D$")
plt.ylabel(r"$I_D / W$ (A/μm)")
plt.title("PMOS: $I_D / W$ vs $g_m / I_D$")
plt.grid(True)
